...
def groupSumClump(start, nums, target):
    ...
    if start >= len(nums):
        ...
        return groupSumClump(i, nums, target - sums) or groupSumClump(i, nums, target)
...
def splitArrayHelper(lst, sum1, sum2, start):
    if start >= len(lst):
        return sum1 == sum2
    else:
        return splitArrayHelper(lst, sum1 + lst[start], sum2, start + 1) \
               or splitArrayHelper(lst, sum1, sum2 + lst[start], start + 1)
...
def splitOdd10Helper(lst, sumOdd, sum10, start):
    if start >= len(lst):
        return sumOdd % 2 == 1 and sum10 % 10 == 0
    else:
        return splitOdd10Helper(lst, sumOdd + lst[start], sum10, start + 1) \
               or splitOdd10Helper(lst, sumOdd, sum10 + lst[start], start + 1)
...
def split53Helper(lst, sum1, sum2, start):
    if start >= len(lst):
        return sum1 == sum2
    else:
        if lst[start] % 5 == 0:
            return split53Helper(lst, sum1 + lst[start], sum2, start + 1)
        elif lst[start] % 3 == 0:
            return split53Helper(lst, sum1, sum2 + lst[start], start + 1)
        else:
            return split53Helper(lst, sum1 + lst[start], sum2, start + 1) \
                   or split53Helper(lst, sum1, sum2 + lst[start], start + 1)
